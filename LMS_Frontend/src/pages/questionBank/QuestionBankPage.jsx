...
import { AlertTriangle, ArrowDown, ArrowDownUp, ArrowUp, Boxes, CheckCircle2, Copy, Download, FileQuestion, FileText, Lock, Pencil, Plus, Trash2, UploadCloud, X } from 'lucide-react';
import { QuestionType, QuestionComplexity, UserRole } from '@/shared';
import { Badge, Button, Card, CardHeader, EmptyState, ErrorState, Input, Modal, Select, SkeletonTable, useConfirm, useToast } from '@/components/ui';
...
function UploadsTab({ moduleId }) {
  ...
  if (!batches || batches.length === 0) {
    return <EmptyState icon={<Boxes size={26} />} title="No uploads yet" description="Excel imports appear here as cards you can review or delete." />;
  }
  return (
...
function BankExcelImport({ moduleId, topics, onClose }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setResult({ added: res?.added ?? 0, duplicateCount: res?.duplicateCount ?? 0 });
    } catch (e2) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      ...
      <p className="lms-muted" style={{ fontSize: 'var(--font-size-sm)', margin: 0 }}>
        {isMcq ? (
          ...
          <>Columns: a <strong>question</strong> column (the scenario / prompt / repo task), <strong>complexity</strong> (easy / medium / hard), an optional <strong>expected answer</strong> (model answer or rubric — improves grading accuracy, never shown to students). Points are set automatically from complexity.</>
        )}
      </p>
